...
const INTRO_FRAGMENT_PATH = "knowledge/AGENTS-intro.md"

async function readIntroFragment(rootDir: string): Promise<string | null> {
  try {
    return await fs.readFile(path.join(rootDir, INTRO_FRAGMENT_PATH), "utf-8")
  } catch {
    return null
  }
}
...
async function collectFragments(rootDir: string): Promise<Array<{ relativePath: string; content: string; fullPath: string }>> {
  ...
  const introPath = path.join(rootDir, INTRO_FRAGMENT_PATH).toLowerCase()
  for await (const fullPath of walk(rootDir)) {
    const relative = path.relative(rootDir, fullPath)
    if (relative === "AGENTS.md") continue
    if (fullPath.toLowerCase() === introPath) continue
...
async function generateSynthBlock(
  ...
  const intro = await readIntroFragment(rootDir)
