...
|---|---|---|---|
| G4 | Generated docs are derived state but tracked in Git | Critical | Fixed |
| G5 | Every docs regeneration creates merge conflicts on unrelated PRs | High | Fixed |
| G6 | `docs:verify-freshness` treats committed blobs as canonical | High | Fixed |
...
- `.gitignore` — `docs/generated/*.md` is ignored; `docs/generated/.gitkeep` preserves the output directory.
- `scripts/verify-documentation-freshness.js` — regenerates projections twice to temporary directories and asserts determinism; no comparison against committed blobs.
- `.github/workflows/docs.yml` — new CI job that runs `docs:generate`, `docs:validate-projections`, and `docs:check-links` on every push to `main` and every pull request, and uploads `docs/generated/` as a build artifact.
- `.github/workflows/publish.yml` — existing continuous-publication workflow already regenerates projections from source and deploys the website; it remains the canonical path for published docs.
- `AGENTS.md` — added a "Derived artifacts" section stating that `docs/generated/*.md` must not be committed and is regenerated by CI.
- `docs/operator/01-getting-started.md` — added a "Generated Documentation" section explaining that projections are CI artifacts and how to inspect them locally.
- `src/governance/derived-files.ts` — already lists `docs/generated/*.md` as a derived path protected from direct edits.
