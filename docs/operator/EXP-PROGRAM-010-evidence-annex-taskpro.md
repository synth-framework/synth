...
- **N7 — Unguarded plan API (I).** `commissionMission`/`chartExpedition` are reachable by any script importing `dist/core/bootstrap.js`, bypassing the CLI's confidence gate. The ExecutionGate governs mutation mechanics, not admission semantics.
