...
Documentation projections under `docs/generated/*.md` are derived from the knowledge base. They are regenerated by CI on every push to `main` and every pull request, and are published as build artifacts. Do not commit these files to version control.
